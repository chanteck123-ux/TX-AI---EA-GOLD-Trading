...
By default, execute in the order “same-risk benchmark → clean up old extensions individually → OFF/OBSERVE consistency → single-module ACTIVE → a few preregistered combinations.” Where a clear complementarity or interaction hypothesis exists, a small number of indicator combinations may also be registered for parallel testing. Individual profitability is not a prerequisite for combination testing; use individual-component and component-removal controls to identify contributions, as detailed in §13.9. When enabling a new module for the first time, do not simultaneously change the lot-sizing algorithm, initial SL/TP, SOP trigger conditions, or exit rules. Provide separate controls for the effectiveness of new methods and the effectiveness of removing old restrictions.
...
4. **Activate candidates separately:** Based on verifiable differences in the training period, test one registered variable at a time by default under §2.2, including core-rule experiments for confirmed NON_USER strategies where appropriate. Separately register interaction combinations meeting §13.9, with individual-component and component-removal controls. NR03 for the user's current Scalping SOP preserves original confirmation/immediate execution. Retain frozen reference and candidate controls, with OFF/OBSERVE checks for applicable supporting modules; do not add ad hoc thresholds to “rescue results.”
...
6. **Combination and promotion:** Prefer individual-module evidence when selecting a small number of two-module combinations. Where a clear interaction hypothesis exists, a small number of combinations may also be tested in parallel under §13.9, without requiring components to be profitable individually first. Treat indicator-combination research separately from promotion of the three-engine Combined system. After freezing the candidate, rerun S-only, I-only, W-only, and actual Combined. Apply §7 and §11 throughout; do not use a combination to conceal a failing individual line.
...
5. **Backtest against the original under the same conditions.** Fix market data, initial capital, base risk, costs, and other conditions; compare the original/module-OFF version against the ACTIVE candidate. By default, change only one main variable at a time; test a small number of combinations with clear interaction hypotheses in parallel under §13.9. Complete the corresponding development backtest after every change. After freezing the candidate, complete real-tick tests, cost/delay stress, and out-of-sample validation not used for parameter tuning. Report differences in net profit USD, equity drawdown, PF, complete-trade count, and costs, retaining missed profits and avoided losses. Report honestly when there are no orders or behavioral differences; do not fabricate improvements.
6. **Continue iteration based on results.** When performance is poor, diagnose indicator use, timing, parameters, excessive filtering, duplicated information, execution implementation, or other causes. Adjust parameters/use or combinations, replace or remove indicators, compile affected source, and rerun backtests. Parameter-only changes may reuse a verified EX5, but the new configuration must be saved and retested. This cycle may repeat; do not skip intermediate tests and claim improved results. Preserve the best version supported by evidence under the current conditions, together with failure records, on every iteration. Keep the original when there is no clear advantage. Combinations must actually be integrated and revalidated; do not add independent results together and call them portfolio performance.
7. **Deliver verifiable artifacts.** For every candidate, retain source, EX5 files, and provenance for the EA and required indicators, SET/INI configurations, compilation logs, integration and decision traces, backtest reports, differences under the same conditions, and the next-iteration decision. Link the corresponding versions and hashes to each run under §13.9. Reuse and distribute external source under its license. Record stages separately as “indicators selected / integrated into candidate code / compiled / backtested / validated / retained or rejected”; completing a plan does not substitute for these stages.
...
### 13.9 Designated Indicator Pool, Autonomous Combinations, and Repeated Validation

On 2026-09-13, the user explicitly requested proactive analysis of uses and combinations for the following 13 indicators, with continued comparison, adjustment, replacement, or rejection after actual integration into an EA. The candidate names below have been checked against the indicator library README and `indicator_manifest.json`; development must still begin by reading the latest versions, interfaces, and validation notes. Slashes denote alternatives or potentially complementary uses to compare. They do not require one selection from each group, simultaneous activation of every indicator, or unanimous agreement before trading.

| Research role | User-specified name → Indicator-library filename (without extension) | Combination and replacement directions to research |
|---|---|---|
| Trend representation | EMA_20_50_100_200 → `EMA_20_50_100_200_MT5`; SMA_Ribbon → `SMA_Ribbon_10_MT5` | Compare representations of moving-average direction, alignment, and slope; check information overlap first, retaining one representation or an evidence-supported complementary use |
| Location and structure | GSM_SNR → `GSM_SNR_MT5`; GSM_SND → `GSM_SND_MT5`; SMC_Structure → `SMC_Structure_MT5` | Compare support/resistance, supply/demand zones, and structural states; interpret each according to its interface, without treating zones, events, and states as the same type of signal |
| Momentum | ZeroLag_MACD → `ZeroLag_MACD_MT5`; CM_MACD_Ult_MTF → `CM_MACD_Ult_MTF_MT5` | Compare the benefits and delays of momentum response and multi-timeframe confirmation; the formulas differ, so do not predetermine which version is best for gold |
| State and timing | Bollinger_RSI_ChartArt → `Bollinger_RSI_ChartArt_MT5`; Stoch_RSI → `Stoch_RSI_MT5`; GSM_ADX_Trend → `GSM_ADX_Trend_Meter` | Research state-dependent triggers or filters with the first two; ADX supplies trend strength, with direction checked separately through DI or structure. Do not mechanically require confluence among all three |
| Volatility scale | Saty_ATR_Levels → `Saty_ATR_Levels_MT5` | Research volatility ranges, location, or distance scales; changes to initial SL/TP or exits still follow strategy ownership and this authorization |
| Volume and price distribution | Volume_Flow_Indicator → `Volume_Flow_Indicator_MT5`; Volume_Profile → `Volume_Profile_MT5` | Research volume confirmation and location within the volume distribution separately; verify tick/real volume and snapshot semantics, without treating the two indicators as equivalent |

1. **Autonomously propose and compare combinations.** Start from current weaknesses and form a hypothesis about which input addresses which problem. Autonomously choose activation, deactivation, replacement, additions/removals, parameters, timeframes, and uses. Begin with individual-component controls by default. Where a clear complementarity or interaction hypothesis exists, a small number of combinations may be registered for parallel testing, even when components have not yet been individually profitable; this does not automatically prohibit combination testing. Where applicable, compare “original, A, B, A+B” and results after removing components individually to identify which components and interactions produce the improvement. Do not exhaustively enumerate every combination without justification; retain experiment registration and per-batch budgets, and record every selection attempt. Indicator-combination research does not remove the individual-strategy and actual Combined promotion requirements in §7 and §11, or expand the scope of the user's current Scalping SOP or base-risk authorization.
2. **The development cycle may repeat, and every change must be tested.** Select or revise a hypothesis → integrate it into EA decision logic → compile affected source and verify EX5 → backtest on registered training/development periods → compare results and decision traces → diagnose weaknesses → adjust uses/parameters/combinations, replace, or reject → compile and test again. Parameter-only changes do not require recompiling unchanged source, but must link the same verified EX5, the new SET/INI, and the new report. Each batch's results support evidence-based research in the next batch. Change direction when a method fails, without blind repetition or ending research after one failure. Continually preserve the best version supported by evidence under the current conditions. Assess “good results” jointly using net profit USD, equity drawdown, PF, complete-trade count, costs, and continued account operation, together with the fixed-anchor composite score and separate acceptance status in §13.10. Do not promise that every candidate will eventually become profitable or that a global optimum has been found.
3. **Freeze after development improvement, then validate for promotion.** For a candidate meeting that round's preregistered research objective, freeze source, EX5, parameters, base risk, and data periods. Then perform the required real-tick tests, cost/delay stress, each-engine and actual Combined tests, and out-of-sample validation on data not used for selection. Checking cost sensitivity early in development does not mean promotion stress tests have passed. If out-of-sample results inform selection or adjustments in the next round, mark that period as used and reserve separate data not used for selection for subsequent validation. Do not keep tuning the same period until it looks good and still call it independent out-of-sample data. If promotion fails, record why and return to the development cycle while preserving the best validated version. If unused data are temporarily unavailable, report the pending-validation status accurately and continue other research that can proceed.
4. **Preserve an implementable research path for Volume Profile.** In the current interface, `shift=0` is the current snapshot of the selected range; it must not be copied onto earlier bars as historical POC/VAH/VAL signals. After inspecting the source, research recalculating and reading the snapshot at each simulated decision time in the Tester using only data available then. First verify the calculation window, refresh behavior, cache, and time semantics. Alternatives include implementing versioned chronological replay/historical buffers or collecting forward snapshots with `available_at`. Where an auditable implementation or records are missing, state the specific historical-data issue, repair that path, and continue testing other indicators. Do not invent an already-existing historical interface or permanently exclude this indicator.
5. **Source, EX5, and results must correspond on every iteration.** Use `candidate_id / run_id` to link the repository commit, SHA256 of EA and dependent-indicator source, EX5 SHA256, SET/INI SHA256, compilation logs, platform and symbol conditions, data versions and periods, cost settings, original-version control, backtest reports, and the explanation of tradeoffs. Recompile after source changes. Failed compilation must not reuse an old EX5 as a new build; failed loading or testing must not reuse an old report as new performance. Rejection stops promotion of the current candidate while preserving the failure reason and recovery evidence. Save GitHub research artifacts within the original access scope and license, without copying private source into a public repository. Retain complete recoverable materials for the current best version; a subsequent evidence-based new use may be registered for testing again.

This saved update defines autonomous combinations and continued validation for the 13 indicators. It does not mean a best combination has been selected or that new EA source, EX5 files, compilation results, or backtest results have been produced. Subsequent research and development must execute and deliver under §§13.8–13.9.

### 13.10 Percentage Composite Score, Comparison, and Iteration

On 2026-09-13, the user requested a 100-point composite score displayed as a percentage, with net profit USD retaining the largest individual weight. The following is this project's research-ranking policy. Its weights reflect the user's current preferences, not statistical facts, profit probabilities, or universal strategy standards. Label it “Composite score 80% (80/100)” in interfaces and reports. The 80% is not a win rate, a probability of future profits, or an automatic passing threshold.

| Scoring component | Weight | Raw metric used for scoring |
|---|---:|---|
| Net profit USD | 35% | Net trading profit P over the same period, with the cost basis reconciled and cash flows such as deposits/withdrawals excluded; do not deduct already-included expenses again |
| Equity drawdown | 25% | Maximum relative equity drawdown D over the full period, using `STAT_EQUITY_DDREL_PERCENT` under §11.2; separately report monetary drawdown and its corresponding percentage |
| PF | 20% | Native MT5 Profit Factor F and the corresponding Gross Profit/Gross Loss; reconcile fees and differences from native net profit under §11 |
| Trade count | 10% | Complete entry-to-exit trade count N over the frozen period, using the aggregation convention in §11; do not inflate the count with partial-exit Deals or Setups |
| Costs | 10% | Cost C per round-trip lot under the same symbol/contract, in USD per round-trip standard lot; calculate consistently as specified below |
| Total | 100% | Always show raw results and component scores together |

**Scoring targets are based on §11.1, “Core Metrics and Assessment.”** Section 11.1 distinguishes mandatory project conditions from diagnostic references and research targets. This section uses fixed anchors to measure performance under the same conditions; it must not turn every reference range into a hard gate or equate “meeting the basic requirement” with “earning full marks.” Candidates that have not yet met the requirements during development may still receive valid diagnostic scores and continue iterating. Display eligibility and validation status separately; only versions meeting the applicable requirements participate in selection of the best eligible version.

| Component | Basis in §11.1 | How to establish formal scoring targets |
|---|---|---|
| Net profit USD | Net profit must be positive; assess capital efficiency together with initial capital, the period, and return on capital | Positive net profit is a basic requirement. Preregister P_target using capital, test duration, and a same-risk benchmark. Do not award full marks merely for becoming profitable or reuse the illustrative amount automatically |
| Equity drawdown | Maximum relative equity drawdown over the full period ≤15% is the target; >15% and ≤30% requires strict manual review; >30% cannot be promoted | Explain D_good/D_bad using these boundaries as a basis, while distinguishing scoring scales from eligibility conditions. Meeting the 15% target does not automatically earn full marks, and a high total score cannot override review or rejection |
| PF | 1.3–1.8 is a diagnostic reference, without a 1.8 upper limit; PF≤1 indicates no positive return edge; review unusually high values under §11.5 | Register F_target before research under the same conditions. Do not make 1.8 the full-score target/upper limit for all strategies or treat the illustrative 2 as the default formal target |
| Complete-trade count | Each line must have >100 trades in the main period, meaning at least 101; 200–500 is recommended, not a trade-count upper limit | N_target must be no lower than the applicable main-period requirement and must be preregistered using strategy frequency and test duration. Cap the score at the scoring target; 101 trades meeting the count requirement does not establish sample reliability or require full marks |
| Costs | Expected Payoff in §11.1 must be assessed against actual costs, with units and reconciliation governed by §11.4 | Preregister C_good/C_bad from the same symbol/contract, broker charges, and frozen cost model, comparing in USD per round-trip standard lot. Also examine after-cost net expectancy; the lowest fees alone do not establish eligibility |

Native Recovery must remain >3. After-cost expectancy, the relationship between win rate and average wins/losses, broker stop-out risk, and other items without separate weights among these five components still require checks under §11.1 and its subsequent detailed provisions. Continue checking ongoing operation and withdrawal scenarios under Section 13. Absence of a separate score does not remove a requirement.

**The saved content defines the scoring rules; formal anchors have not yet been selected.** Section 11.1 does not provide a universal full-score target for net profit USD or cost per round-trip lot, nor does it directly determine full-score values for every component. In subsequent authorized research, register and freeze `P_target / D_good / D_bad / F_target / N_target / C_good / C_bad` using that batch's conditions and available benchmarks. Until the complete scoring policy is registered, mark `SCORE_TARGETS_UNSET`, retain raw metrics and eligibility diagnostics, and do not publish a formal comparable total score. Values below such as 1000 USD and PF 2 are illustrative only; they do not automatically become formal project targets, default configurations, or new risk caps.

1. **Freeze a baseline under the same conditions first.** Before viewing candidate results in each batch, register `score_policy_id / comparison_group_id`, weights, component anchors, scoring targets if any, the complete-trade definition, cost model, validity criteria, and continued-operation criteria. Align initial capital, currency, symbol and contract, market-data period, base risk, sizing mode, and execution/cost scenario. The original, current best, and new candidates use the same policy. Compare different periods, risk tiers, or stress scenarios in separate groups rather than combining them into a directly rankable leaderboard. Do not dynamically scale scores using the minimum/maximum of the current candidate set or move anchors after viewing results to increase scores. A justified new policy may subsequently be registered and applied to recalculate all versions in the same group. Preserve old scores and policies, and do not claim that scores across policies are directly comparable. Scoring criteria must not be reverse-engineered from unseen-sample results.
2. **Convert fixed anchors into component scores.** This version uses the transparent linear mappings below, with every component bounded to 0–100. P_target>0; 0≤D_good<D_bad; F_target>1; N_target is a positive integer no lower than the applicable project's main-period sample requirement; 0≤C_good<C_bad. Target anchors define scoring scales; they do not replace the hard gates in §11 or automatically relax risk. The trade-count score caps at N_target, with no further points for excess trading. Continue reporting raw differences in net profit, PF, and other metrics after their scores have capped.

```text
clip01(x) = min(1, max(0, x))
S_profit = 100 * clip01(P / P_target)
S_dd     = 100 * clip01((D_bad - D) / (D_bad - D_good))
S_pf     = 100 * clip01((F - 1) / (F_target - 1))
S_trades = 100 * clip01(N / N_target)
S_cost   = 100 * clip01((C_bad - C) / (C_bad - C_good))
Score    = 0.35*S_profit + 0.25*S_dd + 0.20*S_pf + 0.10*S_trades + 0.10*S_cost
Display  = Score + "% (" + Score + "/100)"
```

Before scoring, verify that all inputs and anchors are finite, D/F/C are nonnegative, and N is a nonnegative integer; NaN, Infinity, or invalid ranges must not be converted by clip01 into valid scores of 0 or full marks. When P≤0, the net-profit score is 0; when F≤1, the PF score is 0. This does not mean other components can offset an ineligible status. Retain unrounded values during calculation and apply a consistent display precision only at the end. If any required input is missing or undefined, display the composite score as `N/A / INCOMPLETE`. Continue showing known raw metrics and component scores; do not fill missing values with 0/full marks or redistribute weights. Separately identify no trades, no valid round-trip volume, unreconciled costs, a zero PF denominator, an extreme-value platform sentinel, and similar cases. A sample with no losses does not automatically earn a full PF score or a promotion-eligible total score. Finite but unusually high PF remains subject to review under §11.5.

3. **Normalize costs by round-trip volume so fewer trades do not gain an unfair advantage.** Define `RT_lots` as cumulative standard lots with matched, completed entries and exits in the same period: opening 1 lot and then closing 1 lot counts as 1 round-trip lot, not 2. Accumulate matched closed volume for partial exits without splitting one complete trade into several units of N. Allocate reversals, position additions/reductions, and positions crossing period boundaries under frozen trade-matching rules. Report volume and expenses for portions still open separately. `C = CostUSD / RT_lots`; the numerator includes only costs attributable to those matched trade volumes so the scopes agree.

In this version, CostUSD consistently represents nonnegative trading-friction expenses: actual commission/fee charges, negative-swap charges, and spread/adverse-slippage estimates against a frozen execution benchmark without overlap. Report rebates, positive swaps, and favorable slippage separately, while reconciling net profit to actual net amounts. Label each item as “actual charge / estimate embedded in execution price / added stress.” Do not count spread and slippage twice within the same execution deviation or deduct this diagnostic total again from P when P already includes costs. The cost score is an additional preference for execution efficiency and is not statistically independent of net profit; this does not constitute deducting costs from net profit twice. If auditable quotes or a cost breakdown are missing, mark the gap and obtain the evidence; do not award full marks for zero cost. Standard-lot definitions differ across contracts, so C alone must not rank different brokers or symbols. Continue disclosing total costs in USD, cost per complete trade, and open-position expenses. Prolonged pauses or low trade volume do not automatically earn additional points merely because total costs are lower.

4. **Calculation example only.** Suppose the same comparison group preregisters `P_target=1000 USD`, `D_good=5%`, `D_bad=30%`, `F_target=2`, `N_target=200`, `C_good=10 USD/RT_lot`, and `C_bad=30 USD/RT_lot`. A candidate has actual inputs of `P=800 USD`, `D=10%`, `F=1.8`, `N=160`, and `C=14 USD/RT_lot`. Each of the five components scores 80, giving `0.35×80 + 0.25×80 + 0.20×80 + 0.10×80 + 0.10×80 = 80`, displayed as 80%. D and its anchors consistently use percentage-number values (10, 5, 30); do not mix them with 0.10. These illustrative numbers are not results achieved by this project, new risk caps, official targets, or live parameters. These metrics alone do not establish that native Recovery, continued operation, or independent validation has passed.
5. **Keep scores separate from eligibility status.** A high score with nonpositive net profit, insufficient main-period samples, persistent abnormal shutdowns, broker stop-outs, or unmet §11 requirements calls for further diagnosis, adjustment, and validation; the total score cannot justify promotion. Record `score / eligibility_status / validation_status / reason_codes`. Valid diagnostic scores may be retained for ineligible candidates, but must be marked research-only and excluded from the eligible-best-version ranking. Investigate normal periods without trades caused by market filters separately from abnormal shutdowns; do not force entries to increase N. Report continued operation, pause reasons, recovery performance, and withdrawal scenarios separately under §§13.5–13.7. If higher returns come from increased risk, provide separate controls at the same tier rather than calling the gains purely a strategy improvement. Select the current best only among eligible versions in the same group, considering composite scores, raw metrics, and sample quality together.
6. **Include scoring in every round of comparisons.** Repeatedly modify, compile, backtest, and compare under §§13.8–13.9. Show the five raw metrics, component scores, weighted contributions, total score, and differences from the current best on every round. Specify how many more/fewer USD were earned and the changes in drawdown, trade count, and costs. Apply scoring only after results are available for research comparisons, not as an intraday EA signal to add positions or raise risk caps. After meeting preregistered research criteria, freeze the corresponding source, EX5, SET/INI, compilation logs, policy, and reports; then validate across periods, costs, and samples not used for selection. Return to the development cycle when problems are found. Parameter-only changes may reuse a verified EX5, but still require new configurations and backtests. Archive failed versions and retain a recovery point for the current best. Continue the next research round after validation; saving this update does not mean a scoring engine has been implemented or the EA has completed testing.

```mermaid
flowchart TD
    A["EA logic and indicator combinations"] --> B["Integrate or modify candidate source/parameters"]
    B --> C["Compile affected source → Backtest → Compare raw metrics and scores"]
    C -->|Round criteria not met| D["Diagnose, adjust, replace, or reject"]
    D --> B
    C -->|Criteria met| E["Freeze source, EX5, parameters, scoring policy, and reports"]
    E --> F["Validate across periods, costs, and unseen samples"]
    F -->|Problems found| D
