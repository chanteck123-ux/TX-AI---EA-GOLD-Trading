...
2. **开发循环可以重复，每次修改必须测试。** 选择或调整假设 → 接入EA决策逻辑 → 编译受影响源码并核对EX5 → 在登记的训练／开发区间回测 → 比较成绩与决策轨迹 → 查明弱项 → 调整用途／参数／搭配、替换或淘汰 → 再编译和测试。参数单独变化不必重复编译未变源码，但必须关联同一已验证EX5、新SET／INI及新报告。每批结果支持下一批有依据的研究；方法无效就换方向，不盲目重复，也不因一次失败结束。持续保留当前条件下有证据的最佳版本；“成绩好”按净利润USD、净值回撤、PF、完整交易数、成本和账户持续运作共同判断，不能承诺所有候选最终盈利或已经找到全局最佳。
