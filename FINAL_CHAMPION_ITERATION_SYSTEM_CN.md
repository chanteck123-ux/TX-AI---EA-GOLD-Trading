...
1. **选指标并明确用途。** 优先检查用户的[MT5指标库](https://github.com/chanteck123-ux/gsm-mt5-indicators)，按现有问题选择指标，记录版本、参数、周期、数据可用时刻及调用接口。已有合适功能优先复用；缺少所需功能可以在研究候选中实现MQL5模块。每个指标明确参与趋势／震荡识别、信号筛选、风险倍率、进场或退出中的哪一环，不盲目叠加全部指标。
...
5. **与原版同条件回测。** 固定行情、初始资金、基础风险、成本及其他条件，对比原版／关闭模块与ACTIVE候选；首次一次只改变一个主要变量，再研究少量组合。完成真实Tick、成本／延迟压力和未用于调参的样本外验证，报告净利润USD、净值回撤、PF、完整交易数及成本差额；保留错过的盈利与避免的亏损。没有订单或行为差异也如实报告，不能编造改善。
6. **按结果继续迭代。** 表现差就查明指标用途、时序、参数、过滤过严、重复信息或执行实现等原因，调整参数／用法、替换或移除指标，再编译和回测。每轮保留当前条件下有证据的最佳版本及失败记录；没有明确优势先保留原版。单项有效后再接入组合候选重新验证，不把几个独立结果相加当组合成绩。
7. **交付可复核产物。** 每个候选保存EA源码、所需指标源码／EX5与来源、配置SET／INI、编译日志、接入与决策轨迹、回测报告、同条件差异及下一轮决定。外部源码按许可复用和分发。阶段分别记为“已选指标／已接入候选代码／已编译／已回测／已验证／保留或淘汰”，不以计划完成替代这些阶段。
