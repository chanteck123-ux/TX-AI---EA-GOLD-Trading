param(
    ...
    [Parameter(Mandatory)][ValidatePattern('^[A-Za-z0-9_]+$')][string]$CandidateId,
    [string]$FromDate='2026.01.05',
    [string]$ToDate='2026.08.26',
    [ValidateRange(1,10000)][int]$Leverage=100,
    [hashtable]$InputOverrides=@{},
    [int]$TimeoutSeconds=1200
)
$ErrorActionPreference='Stop'
if($Capital -le 0 -or $DelayMs -lt -1 -or $DelayMs -gt 600000){throw 'INVALID_TEST_PARAMETERS'}
$start=[datetime]::ParseExact($FromDate,'yyyy.MM.dd',[cultureinfo]::InvariantCulture)
$finish=[datetime]::ParseExact($ToDate,'yyyy.MM.dd',[cultureinfo]::InvariantCulture)
if($start -ge $finish){throw 'INVALID_TEST_DATE_RANGE'}
...
foreach($key in $InputOverrides.Keys){
    if($key -notmatch '^Inp\w+$' -or "$($InputOverrides[$key])" -match '[\r\n]'){throw 'INVALID_INPUT_OVERRIDE'}
    $parameters[$key]="$($InputOverrides[$key])"
}
...
$end=if($Lane -eq 'UnitTests'){'2026.01.06'}else{$ToDate}
$ini=@('[Experts]','AllowLiveTrading=0','AllowDllImport=0','Enabled=0','[StartUp]','Expert=','Script=','[Tester]',"Expert=$expertName","ExpertParameters=$run.set",'Symbol=GOLD',"Period=$period",'Model=4',"ExecutionMode=$DelayMs",'Optimization=0',"FromDate=$FromDate","ToDate=$end",'ForwardMode=0',"Deposit=$Capital",'Currency=USD',"Leverage=$Leverage", "Report=$run.htm",'ReplaceReport=0','ShutdownTerminal=1','Visual=0','UseLocal=1','UseRemote=0','UseCloud=0')
...
$record['InputOverrides']=$InputOverrides
$record['DelayMode']=if($DelayMs -eq -1){'NATIVE_RANDOM_SECONDS'}else{'FIXED_MILLISECONDS'}
