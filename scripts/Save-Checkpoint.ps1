param([string]$Stage,[string]$Next,[string]$Source='src\GSM_FxPro_R_C00.mq5',[string]$Candidate='R-C00')
...
$record=[ordered]@{Updated=(Get-Date).ToString('o');Stage=$Stage;Candidate=$Candidate;Source=$Source;Purpose='Research only, not Champion';Broker='FxPro';BaselinePackageSHA256=$hash;SourceSHA256=(Get-FileHash -LiteralPath $sourcePath).Hash;Branch=(git -C $root branch --show-current);Next=$Next;HeartbeatId='fxpro-ea';OOS='Historical dates already viewed; not untouched';RealTradingEnabled=$false}
