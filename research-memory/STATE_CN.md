...
AI Alpha架构保存（2026-09-20）：中英文计划新增§13.11，保存研究编排、MQL5独立执行、已有仓位保护与新入场门控分离、优化后重新测试／复审，以及冻结验证、Demo版本对应和发布身份区分。原始草案存于[参考档案](../docs/reference/AI_ALPHA_RESEARCH_SYSTEM_MT5_ORIGINAL_CN_20260920.txt)，其Python、评分和示例数值以已修正计划为准。状态为DESIGN_SAVED／NOT_IMPLEMENTED／NOT_TESTED；本次未启动Agent服务、EA开发／编译／回测、Demo或实盘，未改变Champion，原评分锚点未补设。此记录不重验或覆盖下列历史状态。

评分目标依据澄清（2026-09-13）：§13.10已补§11.1五项指标的目标来源映射，明确基本达标不等于满分、未计权指标仍须核对，演算全组锚点未被选为正式值。正式政策未登记时总分N/A／SCORE_TARGETS_UNSET，仍可继续原始指标诊断及可执行研究；本次仅保存衔接说明，权重、公式、§11判定及原验收均未改动。

综合评分补充（2026-09-13）：中英文§13.10保存净利／净值回撤／PF／完整交易数／成本的35／25／20／10／10权重、固定锚点公式、80分演算例子及迭代流程图。评分百分比不表示胜率，独立于§11资格与验证状态；成本按匹配往返手数归一，缺失数据不填零。当前仅完成评分设计、文档与个性化保存，未实现评分程序，未运行EA编译或回测，演算数据不是实际策略成绩。

指标搭配补充（2026-09-13）：中英文计划§13.9已列明用户指定的13项指标及研究角色，并修正原执行入口。允许自主选择、比较和替换；有明确交互假设的少量组合无需等待组件单独盈利。每次修改后须对应编译／配置与新回测，在开发区间反复改进，候选冻结后再进行独立样本外等晋级验证。本次已读取指标库README和清单，未重新编译或验证其中二进制；仅保存计划和个性化，没有产生最佳搭配、EA源码／EX5或新的回测成绩。

开发交付澄清（2026-09-12）：§13.8及§1/§13入口已明确有用指标需实际接入EA候选，编译、回测、比较和迭代；OBSERVE仅是工程前置，不能要求接入前先证明盈利。此次保存了该要求，既有实现／测试状态仍按下文实际证据记录。

最新计划补充（2026-09-12）：用户要求加入动态风险调整、趋势／震荡识别、黄金／美元关联市场以及新闻和经济日历分析，并同步个性化与中英文计划。第13章及原执行入口已更新；通用官方接口已核实，本账户品种、build、新闻权限和历史快照未连接核对。DR01／NR07／NR08及NR02扩展均为PLANNED、NOT_IMPLEMENTED、NOT_TESTED，未产生新策略成绩或部署变化。

计划入口更新：2026-09-12。本次没有实现 Candidate、编译 EA、运行 MT5 回测或更改 Champion。下文 2026-09-10 的事实和证据缺口保留其历史日期；需要执行时重新核对当前环境与证据。

同日用户补充：当前这套Scalping SOP属于用户原创；确认非用户原创策略可以自主优化和改善。具体来源分类、核心规则修改范围及实验边界已同步到中英文计划§2.2；Intraday/Swing的归属没有在本次自动确定。此次仍仅更新文档。
