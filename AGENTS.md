...
开发中的指标研究必须落实为EA候选的实际接入、真实编译、回测及继续迭代，详见当前计划§13.8。数据／接口与OBSERVE工程检查通过、有合理研究假设后即推进ACTIVE离线候选；不要求接入前先证明盈利，不以长期观察、只显示指标或只写报告作为开发完成。每轮对照原版，保留有依据的最佳版本，修正、替换或淘汰弱项；用户Scalping SOP范围仍按§2.2。
