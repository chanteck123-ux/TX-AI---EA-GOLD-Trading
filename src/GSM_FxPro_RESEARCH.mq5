...
#property description "FxPro 3-SOP RESEARCH. Intraday runner optional. Scalping fixed exits. Not Champion; real-account execution disabled."
...
input MoneyManagementMode InpMoneyManagementMode = MONEY_AUTO_RISK;
...
input SmallAccountProfile InpSmallAccountProfile = SMALL_ACCOUNT_OFF;
...
input bool   InpUseFixedLot              = false;     // Research risk budget, never force the broker minimum.
...
#include "StrictRiskResearch.mqh"
#include "IntradayRunner.mqh"
#include "StudyEvidence.mqh"

double EffectiveStrategyRiskPercent(StrategyId strategy,double configuredRisk)
{
   configuredRisk=MathMin(configuredRisk,InpResearchSingleRiskPct);
   if(InpCapitalLadderMode!=CAPITAL_LADDER_ENFORCE)
...
{
   return InpResearchTotalRiskPct;
   /* Historical caps are superseded only in this research baseline.
...
int OnInit()
{
   studyRunnerMode=InpIntradayRunner;
   if(StringFind(AccountInfoString(ACCOUNT_SERVER),"FxPro")<0 ||
      !MathIsValidNumber(InpRunnerBreakEvenR) || InpRunnerBreakEvenR<=0 ||
      !MathIsValidNumber(InpRunnerTrailStartR) || InpRunnerTrailStartR<InpRunnerBreakEvenR ||
      !MathIsValidNumber(InpRunnerATRMultiple) || InpRunnerATRMultiple<=0 || InpRunnerATRPeriod<2 ||
      (InpRunnerRestartProbe && !MQLInfoInteger(MQL_TESTER)))
   {
      Print("FXPRO_RESEARCH_INIT_REJECT: server or runner parameters invalid");
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!RiskNumber(InpResearchSingleRiskPct) || InpResearchSingleRiskPct<=0.0 || InpResearchSingleRiskPct>1.0 ||
      !RiskNumber(InpResearchTotalRiskPct) || InpResearchTotalRiskPct<=0.0 || InpResearchTotalRiskPct>3.0 ||
      !RiskNumber(InpResearchRoundTripFeePerLotUSD) || InpDeviationPoints<0 || InpScalpUseBreakEven ||
      InpMoneyManagementMode!=MONEY_AUTO_RISK || InpUseFixedLot || InpForceBrokerMinimumLot ||
      InpEnableConfidence100LotBoost || InpSmallAccountProfile!=SMALL_ACCOUNT_OFF ||
      !InpRequireHedging || AccountInfoString(ACCOUNT_CURRENCY)!="USD")
   {
      Print("R-C00_INIT_REJECT: mandatory risk settings, USD hedging, automatic sizing and Scalping fixed exits required");
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!MQLInfoInteger(MQL_TESTER) && AccountInfoInteger(ACCOUNT_TRADE_MODE)==ACCOUNT_TRADE_MODE_REAL)
   {
      Print("R-C00_RESEARCH_ONLY: real-account execution disabled");
      return INIT_FAILED;
   }
   Print("FxPro 3-SOP research 4.23, R-C01 control. NOT CHAMPION. Gaps can exceed planned SL risk.");
   if(!ValidateInputs())
      return INIT_PARAMETERS_INCORRECT;
   ...
   if(!RefreshSymbolSpec())
      return INIT_FAILED;
   if(SymbolInfoString(g_symbol,SYMBOL_CURRENCY_PROFIT)!="USD")
   {
      Print("FXPRO_RESEARCH_INIT_REJECT: gold profit currency must be USD");
      return INIT_FAILED;
   }
   ...
   PrintFormat("GSM黄金三策略EA v4.23研究版已启动（未批准实盘）：%s，Hedging=%s，Digits=%d Point=%g TickSize=%g MinLot=%g MaxLot=%g Step=%g Stops=%d Freeze=%d",
...
{
   if(!g_strategyEnabled[strategy]) return "ENGINE_DISABLED: 本轮不交易此策略";
   if(g_funnel[strategy].ordersFilled>0) return "已有成交";
   if(g_funnel[strategy].ordersRequested==0 && g_funnel[strategy].riskRejected>0)
      return StringFormat("EXECUTION_RISK_GATE: 风险拒绝=%I64d 最小手数超预算=%I64d 总风险拒绝=%I64d；没有发送券商订单",
                          g_funnel[strategy].riskRejected,g_funnel[strategy].minimumLotRiskRejected,g_funnel[strategy].totalRiskRejected);
                        ...
                        double riskPercent)
{
   ...
   bool lotReady=false;
   studyRunnerSizing=(InpIntradayRunner && magic==InpIntradayMagic);
   lotReady=ResearchSize(dir,entry,sl,riskPercent,lots,calculatedLots,lotReason);
   if(!lotReady || lots<g_volumeMin || lots>g_volumeMax)
   {
      ...
      if(StringFind(lotReason,"MINIMUM_LOT_EXCEEDS_RISK")>=0)
      {
   ...
   if(exactRiskReady)
   {
      ...
      bool enforceStrategyCap=true;
      if(enforceStrategyCap && projectedRiskPct>MathMin(strategyRiskCap,InpResearchSingleRiskPct)+1e-8)
      {
   ...
   if(!SymbolInfoTick(g_symbol,latest) ||
      !ResearchValidateOrder(dir,(dir>0 ? latest.ask : latest.bid),sl,lots,riskPercent,lotReason))
   {
      g_runtime[strategy].rejectReason="STRICT_RISK_RECHECK: "+lotReason;
      g_funnel[strategy].riskRejected++;
      return false;
   }
   ...
   uint brokerCode=trade.ResultRetcode();
   bool filled=(ok && (brokerCode==TRADE_RETCODE_DONE || brokerCode==TRADE_RETCODE_DONE_PARTIAL) && trade.ResultDeal()>0);
   if(!filled)
   {
      if(brokerCode==TRADE_RETCODE_PLACED || brokerCode==TRADE_RETCODE_TIMEOUT ||
         brokerCode==TRADE_RETCODE_CONNECTION || brokerCode==TRADE_RETCODE_DONE || brokerCode==TRADE_RETCODE_DONE_PARTIAL)
         researchExecutionUnresolved=true;
...
{
   return; // Hard isolation: this research EA never protects Scalping profits.
}

void ManageIntradayPosition()
{
   if(InpIntradayRunner) { ManageIntradayRunner(); return; }
...
double CalculateAccountOpenRiskPercent()
{
   double reserved=0.0;
   string reason="";
   double strictEquity=AccountInfoDouble(ACCOUNT_EQUITY);
   if(strictEquity<=0.0 || !ResearchReservedRisk(reserved,reason))
   {
      static string lastReason="";
      static datetime lastLog=0;
      if(reason!="" && (reason!=lastReason || TimeCurrent()-lastLog>=60))
      {
         Print("STRICT_ACCOUNT_RISK_BLOCK|",reason);
         lastReason=reason;
         lastLog=TimeCurrent();
      }
      return 100.0;
   }
   return reserved/strictEquity*100.0;
   /* V4.00 used only current SL risk, allowing BE to release the whole budget.
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   if(equity <= 0.0)
      return 100.0;
...
{
   StudyTransaction(trans,request,result);
...
void CreateDashboardObjects()
{
   ...
   ConfigureDashboardLabel(DashboardObjectName("TITLE"),x+16,y+10,MathMax(14,InpPanelFontSize+4),C'244,203,111',"FxPro GOLD 4.23  |  RESEARCH / NOT LIVE",20);
   ...
   ConfigureDashboardLabel(DashboardObjectName("FOOTER"),x+16,y+472,MathMax(8,InpPanelFontSize-1),C'167,178,190',"开关控制新仓；Scalping仅固定SL/TP。Intraday/Swing按各自参数管理。",20);
}
