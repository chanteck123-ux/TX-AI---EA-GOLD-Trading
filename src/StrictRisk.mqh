#ifndef GSM_RESEARCH_STRICT_RISK
#define GSM_RESEARCH_STRICT_RISK
...
bool ResearchReservedRisk(double &money,string &reason)
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      double reserved=RiskReservation(initial,MathMax(0.0,-current/g_volumeMin),
...
bool ResearchUnitRisk(const int dir,const double entry,const double sl,double &unit,string &reason)
{
   ...
   unit=-pnl/g_volumeMin+InpResearchRoundTripFeePerLotUSD;
                   ...
                   double &lots,double &raw,string &reason)
{
   ...
   raw=MathMax(0.0,budget/unit);
   lots=RiskBudgetVolume(equity,single,InpResearchTotalRiskPct,reserved,unit,g_volumeMin,g_volumeMax,g_volumeStep);
   if(lots<g_volumeMin)
   {
      reason=StringFormat("MINIMUM_LOT_EXCEEDS_RISK_OR_NO_BUDGET Equity=%.2f Budget=%.4f MinRisk=%.4f TechnicalMinEquity=%.2f Reserved=%.4f",
                          equity,budget,g_volumeMin*unit,g_volumeMin*unit/(single/100.0),reserved);
      return false;
   }
   return true;
}
                           ...
                           const double requestedRisk,string &reason)
{
   ...
   double equity=AccountInfoDouble(ACCOUNT_EQUITY);
   double single=MathMin(InpResearchSingleRiskPct,requestedRisk);
   if(!RiskBudgetFits(equity,single,InpResearchTotalRiskPct,reserved,volume*unit))
   {
      reason="FRESH_QUOTE_OR_EQUITY_EXCEEDS_RISK_BUDGET";
      return false;
   }
   PrintFormat("STRICT_RISK_PLAN|Equity=%.2f|SingleCapPct=%.4f|Volume=%.8f|PlannedUSD=%.4f|PlannedPct=%.4f|ReservedUSD=%.4f|TotalPct=%.4f|FeeEstimateUSDPerLot=%.4f|AdverseBufferPoints=%d|InitialMarginUsed=%.4f",
               equity,single,volume,volume*unit,volume*unit/equity*100.0,reserved,
               (reserved+volume*unit)/equity*100.0,InpResearchRoundTripFeePerLotUSD,
