#ifndef GSM_RESEARCH_STRICT_RISK_ROUNDED
#define GSM_RESEARCH_STRICT_RISK_ROUNDED
                   ...
                   double &lots,double &raw,string &reason)
{
   ...
   if(lots<g_volumeMin)
   {
      double minimumRisk=RiskRoundedCost(unit,g_volumeMin,InpResearchRoundTripFeePerLotUSD,digits);
                           ...
                           const double requestedRisk,string &reason)
{
   ...
   double single=MathMin(InpResearchSingleRiskPct,requestedRisk);
   int digits=(int)AccountInfoInteger(ACCOUNT_CURRENCY_DIGITS);
   double fee=RiskRoundedFee(volume,InpResearchRoundTripFeePerLotUSD,digits);
   double proposed=RiskRoundedCost(unit,volume,InpResearchRoundTripFeePerLotUSD,digits);
   ...
   PrintFormat("STRICT_RISK_PLAN|Equity=%.2f|SingleCapPct=%.4f|Volume=%.8f|PlannedUSD=%.4f|PlannedPct=%.4f|ReservedUSD=%.4f|TotalPct=%.4f|FeeEstimateUSDPerLot=%.4f|AdverseBufferPoints=%d|InitialMarginUsed=%.4f|FeeModel=PER_SIDE_CEILING|CurrencyDigits=%d|FeeBudgetUSD=%.8f|LinearFeeUSD=%.8f",
               ...
               InpDeviationPoints,AccountInfoDouble(ACCOUNT_MARGIN),digits,fee,volume*InpResearchRoundTripFeePerLotUSD);
   return true;
}
