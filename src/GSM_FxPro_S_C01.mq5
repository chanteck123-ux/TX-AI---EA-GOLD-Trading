...
#property description "S-C01 research: Scalping distance-only zone ranking. Not Champion; real-account execution disabled."

#include <Trade/Trade.mqh>
#include "ZoneRank.mqh"
...
int OnInit()
{
   ...
   Print("R-C00 risk-normalized baseline, not Champion. Costs estimated; gaps can exceed planned SL risk.");
   if(!ValidateInputs())
      return INIT_PARAMETERS_INCORRECT;
                    ...
                    Zone &best)
{
   ...
   for(int i = 3; i <= maxIndex; i++)
   {
      ...
      if(ZoneRankBetter(strategy==STRATEGY_SCALPING,best.valid,
                        dist,bestDistance,z.score,best.score))
      {
         best = z;
         bestDistance=dist;
      }
