...
$receiptPath=$null; $code=1
$state=[ordered]@{version='1.0.1';status='RUNNING';source='NOT_STARTED';data='NOT_STARTED';compile='NOT_STARTED';backtests=@();error=$null;live='DISABLED';mt5='NOT_ACCESSED'}
...
function Audit-Quotes([string]$ZipPath,[string]$AuditPath){
    ...
    if($actualBlob -ne $script:quoteBlob){throw 'XAUUSD sample differs from pinned data; no silent substitution.'}
    ...
    try{
        ...
        if($entries.Count -ne 1){throw 'Expected exactly one historical XAUUSD CSV.'}
        $reader=New-Object IO.StreamReader($entries[0].Open())
        try{
            while($null -ne ($line=$reader.ReadLine())){
                if([string]::IsNullOrWhiteSpace($line)){continue}
                $parts=$line.Split(',');$t=[DateTime]::ParseExact($parts[0],'yyyyMMdd HH:mm',$script:culture)
                $key=[string]$parts.Length
                if(-not $fieldCounts.ContainsKey($key)){$fieldCounts[$key]=0};$fieldCounts[$key]++
                if($parts.Length -ne 11){throw 'Expected bid/ask OHLC columns; trade-bar substitutes are not accepted.'}
    ...
    if($count -lt 1000 -or $duplicates -gt 0 -or $outOfOrder -gt 0 -or $badQuotes -gt 0){throw 'Historical quote audit failed.'}
    $end=$last.Date;$start=$end.AddYears(-2)
    ...
    $audit=[ordered]@{
        ...
        git_blob_sha1=$actualBlob;sha256=(Hash $ZipPath);resolution='H1'
        timestamp_field='Bar start in the LEAN data-file timezone; not assumed UTC'
        ...
        gaps_gt_2h=$gaps;gap_note='Includes market closures/weekends; not a trading-calendar completeness certificate.'
        ...
        selection_rule='Latest available 24 calendar months excluding last potentially partial date; selected by coverage before returns.'
        limitations=@('Historical bundled sample, not current data','Not FxPro','Not real ticks','No historical news/calendar/USD inputs','No synthetic replacement quotes')
    }
    Write-Json $audit $AuditPath;return $audit
}
try{
    if(-not $LeanRoot){$profileHome=$env:USERPROFILE;if(-not $profileHome){$profileHome=$HOME};$LeanRoot=Join-Path (Join-Path $profileHome 'QuantConnect-LEAN-Local') 'Lean'}
    ...
    if((Test-Path -LiteralPath $OutputRoot) -and @(Get-ChildItem -LiteralPath $OutputRoot -Force).Count -gt 0){throw 'Output is not empty; previous evidence will not be overwritten.'}
    ...
    if($head -ne $commit){throw 'Pinned LEAN commit mismatch.'}
    $dirty=(Native $git @('-C',$LeanRoot,'status','--porcelain')).Trim()
    if($dirty){throw 'LEAN tree is modified; existing work was left intact.'}
    ...
    $audit=Audit-Quotes $quotePath (Join-Path $OutputRoot 'data_audit.json');$state.data='PINNED_HOURLY_SAMPLE_AUDITED';Save-State
    $prereg=[ordered]@{
        authorization='Independent LEAN gold research; not an MT5 SOP/EA modification.'
        initial_cash_usd=1000;risk_fraction=0.005;leverage_model=20;defaults_status='New research starting settings, not approved live limits.'
        ...
        target='H1-close-triggered market exit at favorable move >=2R or >=3R, not resting TP.'
        session='08:00 <= completed-bar New York time <15:00 entry; flat at first available close >=16:00.'
        position='One position, no adding, martingale, BE, trailing or partial exit; actual unit step, risk and margin sizing.'
        base_cost='Quoted bid/ask, USD0.035/unit/side commission, USD0.05 absolute price slippage/side. Model assumptions, not verified broker tariff.'
        slippage_accounting='Legacy metrics field modeled_slippage_usd_already_in_fills is a configured allowance, NOT separately measured actual slippage. Native fills drive PnL; never deduct this allowance again.'
        comparison='A2R vs B3R, then actual rerun of each with 2x commission and 2x slippage; same cash, risk, entry, SL and dates.'
        no_claims=@('No OOS','No tick DD','No MT5/FxPro/live/withdrawal validation','No formal score without anchors')
        previous_failed_run='35519872021: incorrect QuantConnect.dll reference; corrected to actual QuantConnect.Common.dll. No completed performance backtest in that failed attempt.'
    ...
    if(-not (Test-Path -LiteralPath $launcher -PathType Leaf)){throw 'Compiled LEAN launcher missing.'}
    ...
    $refs=@('QuantConnect.Common','QuantConnect.Algorithm','QuantConnect.Indicators','QuantConnect.Brokerages','NodaTime')
    ...
    $candidateProject=Join-Path $build 'IndependentLeanGold.csproj';[IO.File]::WriteAllText($candidateProject,$projectXml,[Text.UTF8Encoding]::new($false))
    $null=Native $dotnet @('build',$candidateProject,'-c','Release','--verbosity','minimal') (Join-Path $OutputRoot 'candidate-build.log')
    $dll=Join-Path $build 'bin/Release/net10.0/IndependentLeanGold.dll';if(-not (Test-Path -LiteralPath $dll)){throw 'Compiled candidate DLL missing.'}
    ...
    foreach($e in $experiments){
        $run=Join-Path $OutputRoot $e.id;$nativeOut=Join-Path $run 'native';New-Item -ItemType Directory -Path $nativeOut -Force|Out-Null
        $config=[ordered]@{
            environment='backtesting';'algorithm-id'=$e.id;'algorithm-type-name'='GoldResearchAlgorithm';'algorithm-language'='CSharp'
            'algorithm-location'=$dll;'data-folder'=(Join-Path $LeanRoot 'Data');'results-destination-folder'=$nativeOut
        ...
        $configPath=Join-Path $run 'config.json';Write-Json $config $configPath;Set-Location -LiteralPath $bin
        $null=Native $dotnet @($launcher,'--config',$configPath) (Join-Path $run 'engine.log')
        $metricsPath=Join-Path $run 'metrics.json';if(-not (Test-Path -LiteralPath $metricsPath)){throw ('Missing metrics: '+$e.id)}
        ...
        if(-not $m.cashflow_reconciliation_pass -or $m.final_open_quantity -ne 0 -or $m.closed_round_trips -lt 1 -or
            $m.rejected_orders -ne 0 -or @(Get-ChildItem -LiteralPath $nativeOut -Filter '*.json').Count -lt 1){throw ('Incomplete run evidence: '+$e.id)}
        $results.Add($m);$state.backtests+=@{id=$e.id;status='COMPLETED';metrics_sha256=(Hash $metricsPath);config_sha256=(Hash $configPath)};Save-State
    }
    ...
    $rows.Add('固定官方源码随附的 OANDA 来源 XAUUSD H1 历史样本，不是近期/FxPro真实Tick；不称样本外。')
    ...
    $rows.Add('')
    $rows.Add('A/B只改变H1收盘止盈门槛2R/3R。COST2X分别加倍佣金与价格滑点后实际重跑；仓位仍按同一风险预算重新计算，不是固定成交路径直接扣费。')
    $rows.Add('净利润按原生实际模型成交价及佣金核算。点差、原生模型施加的滑点已在成交价中，不重复扣除。配置滑点预算不是单独测量的实际滑点；旧JSON字段 modeled_slippage_usd_already_in_fills 按此更正解释。')
    $rows.Add('小时K线不能重建Tick顺序、盘中最大净值DD和券商延迟。止损是原生StopMarket；止盈是等收盘再市价退出，不保证精确2R/3R。')
    $rows.Add('隔夜费未计；检查 overnight_date_transitions_while_holding，非0必须补成本模型。没有MT5/FxPro/实盘/提款持续账户验证。')
    $rows.Add('总分N/A，SCORE_TARGETS_UNSET。回测完成不代表策略达标或Champion。')
    ...
    Set-Location -LiteralPath $original.Path;Write-Host ('FINAL STATUS: '+$state.status)
