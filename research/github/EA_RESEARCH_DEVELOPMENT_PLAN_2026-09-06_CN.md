...
| 编号 | 可选计划 | 工作范围 | 计划交付内容 |
|---|---|---|---|
| 1 | **旧版升级研究计划** | 以现有R-C01工程为基础，检查问题，逐项修复和优化；保留旧版作对照 | 旧版问题与差异清单、升级顺序、独立候选和回归／对照测试计划 |
| 2 | **codex重新研究开发计划** | 由Codex依据已确认SOP与项目要求，重新研究策略结构、模块接口和实现方案；在独立研究分支保存新候选，以R-C01作比较基线 | 重新研究后的架构方案、模块开发任务、实验顺序、测试与验收计划 |
| 3 | **研究开发计划指南** | 整理如何开展EA研究开发的方法与操作步骤，供制定计划和接续工作使用 | 研究流程、实验登记方法、结果解读、交付清单和续做检查点指南 |

后续可直接用编号1／2／3或完整名称选择。每轮记录一个主路线及其适用范围；切换路线时保留原计划版本与检查点。选择记录包含：**路线名称、选择日期、适用范围、关联计划版本及基线hash**。

**本次仅新增选择入口，尚未指定切换路线；现有R-C01安排继续保留。** 三条路线共同沿用本文已确认的SOP、风险、报告和Champion验收要求。“重新研究”可重新设计实现方案，研究候选与历史基线分别保存并验证；“指南”提供操作方法。本次新增选项没有启动代码重写、编译或回测。
